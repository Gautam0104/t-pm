    ...
    .then(users => {
      ...
      const modifiedResults = users.map(user => {
        ...
        switch (user.role_id) {
          ...
          default:
            roleText = `<i class="ti ti-user ti-md text-info me-2"></i><span class=" me-1">${user.role_name}</span>`;
        }
      ...
      modifiedResults.map(element => {
        ...
        const month = (datecreate.getMonth() + 1).toString().padStart(2, "0"); // Months are 0-based
        const year = datecreate.getFullYear();

        // Extract time components
        const hours = datecreate.getHours().toString().padStart(2, "0");
        const minutes = datecreate.getMinutes().toString().padStart(2, "0");
        const seconds = datecreate.getSeconds().toString().padStart(2, "0");
...
const tableBody = document.getElementById(ELEMENT_IDS.USER_TABLE_BODY);
...
const userRole = async () => {
  return await fetch(`${API_BASE_URL}${API_ROUTES.GET_ROLES}`)
    ...
    .then(data => {
      const roleSelect = document.getElementById(ELEMENT_IDS.USER_ROLE);
      data.map(role => {
        const roleOption = `
...
userRole();
// create new user
const registerForm = document.getElementById(ELEMENT_IDS.ADD_NEW_USER_FORM);
const messageDiv = document.getElementById(ELEMENT_IDS.MESSAGE);
...
registerForm.addEventListener("click", async e => {
  ...
  const username = document.getElementById(ELEMENT_IDS.ADD_USER_EMAIL).value;
  const password = document.getElementById(ELEMENT_IDS.ADD_USER_PASSWORD).value;
  const first_name = document.getElementById(ELEMENT_IDS.ADD_USER_FIRSTNAME)
    .value;
  const last_name = document.getElementById(ELEMENT_IDS.ADD_USER_LASTNAME)
    .value;
  const role_id = document.getElementById(ELEMENT_IDS.USER_ROLE).value;

  try {
    const response = await fetch(`${API_BASE_URL}${API_ROUTES.AUTH_REGISTER}`, {
      method: "POST",
...
export const editUser = async user_id => {
  try {
    ...
    user.map(ele => {
      const updateUserForm = document.getElementById(ELEMENT_IDS.UPDATE_USER);
      const formContent = `
                                <div class="mb-6">
      ...
      fetch(`${API_BASE_URL}${API_ROUTES.GET_ROLES}`)
        ...
        .then(data => {
          const updateroleSelect = document.getElementById(
            ELEMENT_IDS.UPDATE_USER_ROLE
          );
          data.map(role => {
            const roleOption = `
                                <option value="${role.role_id}">${role.role_name}</option>
      ...
      document
        .getElementById(ELEMENT_IDS.UPDATE_USER_FORM)
        .addEventListener("click", async e => {
          ...
          const first_name = document
            .getElementById(ELEMENT_IDS.UPDATE_USER_FIRSTNAME)
            .value.trim();
          const last_name = document
            .getElementById(ELEMENT_IDS.UPDATE_USER_LASTNAME)
            .value.trim();
