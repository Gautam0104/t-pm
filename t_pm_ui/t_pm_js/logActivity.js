...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (ticketForm) {
    ticketForm.addEventListener("input", event => {
      ...
      logActivity(
        `${userName} updated ${event.target.name ||
          "a field"} in the ticket form`
      );
    });
...
document.addEventListener("click", event => {
  ...
  logActivity(
    `Clicked on ${event.target.tagName} ${event.target
      .className} at (${event.clientX}, ${event.clientY}) at ${currentTime}`
  );
});
